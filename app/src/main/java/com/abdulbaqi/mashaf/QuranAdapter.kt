...
class QuranAdapter(
    ...
    private inner class AyahVH(private val b: ItemAyahBinding) : RecyclerView.ViewHolder(b.root) {
        ...
        fun bind(item: QItem.Ayah) {
            ...
            // ✅ حساب رقم العرض (إن كانت البسملة أول عنصر في السورة لغير الفاتحة: العد يبدأ من الآية التالية)
        ...
        private fun applyAllColors(text: String, ornateNumberPart: String?): SpannableString {
            ...
            // (B) تلوين "الله" فقط بالأخضر (يشمل اللَّه / ٱللَّه مع الحركات)
            // شرط مهم: لا يلوّن إذا كان قبلها حرف عربي (يعني لا يلوّن: بالله / لله / والله ...)
            val allahPattern = Regex("(?<![\\u0600-\\u06FF])([ٱا]$DIACRITICS*ل$DIACRITICS*ل$DIACRITICS*ه$DIACRITICS*)(?![\\u0600-\\u06FF])")
            ...
            // (C) تلوين "إسرائيل" و"اسرائيل" و"اليهود" بالأحمر (مطابقة مباشرة)
