...
class QuranAdapter(
    ...
    // ✅ خريطة اسم السورة مرة واحدة (بدل البحث داخل القائمة لكل آية)
    private val surahNameByIndex: Map<Int, String> = buildMap {
        for (it in items) {
            if (it is QItem.SurahTitle) put(it.surahIndex, it.name)
        }
    }

    // ✅ كاش للنص النهائي الملوّن لتقليل الحمل أثناء التمرير
    // المفتاح: (surahIndex, ayahIndex, hash(text)) في Long واحد
    private val spanCache = object : LruCache<Long, SpannableString>(800) {}
    ...
    private fun makeKey(s: Int, a: Int, h: Int): Long {
        // pack: s (16 bits) + a (16 bits) + h (32 bits)
    ...
    private inner class AyahVH(private val b: ItemAyahBinding) : RecyclerView.ViewHolder(b.root) {
        ...
        fun bind(item: QItem.Ayah) {
            ...
            val cacheKey = makeKey(item.surahIndex, item.ayahIndex, cleanText.hashCode())
            val cached = spanCache.get(cacheKey)
            if (cached != null) {
                b.tvAyah.text = cached
                return
            }

            val ss = buildSpannable(item, surahName, cleanText)
            spanCache.put(cacheKey, ss)
            b.tvAyah.text = ss
        }

        private fun buildSpannable(item: QItem.Ayah, surahName: String, cleanText: String): SpannableString {
